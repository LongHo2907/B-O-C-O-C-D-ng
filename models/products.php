...
class Product extends DB implements Iproduct
{
    ...
    function insert($payload, $srcs, $srcOfContent)
    {
        try {
            ...
            $stm = $this->db->prepare('INSERT INTO ' .
                self::tableName . '(name,price,brand_id,keyword,short_desc,quantity_product,model,chip,ram,card,drive,display,connect,operation,pin,weight,size, time_add)
                            VALUES(:productName,:price,:brand_id,:keyword,:short_desc,:quantity,:model,:chip,:ram,:card,:drive,:display,:connect,:operation,:pin,:weight,:size, :time_add)');
            $stm->execute(array(
    ...
    function update($payload)
    {
        try {
            ...
            ,short_desc=:short_desc,quantity_product=:quantity,model=:model,chip=:chip,ram=:ram,card=:card,drive=:drive
            ,display=:display,connect=:connect,operation=:operation,pin=:pin,weight=:weight,
