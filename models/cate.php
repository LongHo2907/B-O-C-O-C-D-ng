...
class Cate extends DB implements Icate
{
    ...
    function getCateProduct($product_id) //get  cate_id
    {
        $stm = $this->db->prepare("SELECT cp.cate_id FROM cate_product cp INNER JOIN product p ON cp.product_id= p.id INNER JOIN category c ON c.id= cp.cate_id WHERE cp.product_id=$product_id limit 1" );
        $stm->setFetchMode(PDO::FETCH_ASSOC);
        $stm->execute();
        return $stm->fetch();
    }
    ...
    function getCatesByProductId_id($product_id)
    {
        $stm = $this->db->prepare("SELECT c.id, c.name FROM category c INNER JOIN cate_product cp ON cp.cate_id= c.id INNER JOIN product p ON cp.product_id= p.id WHERE cp.product_id=$product_id ");
        $stm->setFetchMode(PDO::FETCH_ASSOC);
