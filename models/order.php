...
class Order extends DB
{
    ...
    { //in admin order
        $stm = $this->db->prepare('SELECT o.id,o.time,o.status,o.notes,u.username, u.full_name FROM ordered o INNER JOIN user u ON u.id = o.user_id
    ...
    function insert($payload, $cart)
    {
        try {
            ...
            $extra_address = $payload['shipping_address'];
            $notes = $payload['shipping_notes'];
            $name_get = $payload['shipping_full_name'];
            $sdt_get = $payload['shipping_phone'];
            //
            $stm = $this->db->prepare('INSERT INTO ' .
                self::tableName . '(user_id,status,notes,extra_address, time, name_get, sdt_get)
                 VALUES(:user_id, :status, :notes, :extra_address, :time, :name_get, :sdt_get)');
            $stm->execute(array(
                'user_id' => $user_id,
                'status' => 0,
                'notes' => $notes,
    ...
    function getOrderById_id($id)
    {
        $stm = $this->db->prepare("SELECT * FROM ordered o INNER JOIN user u ON o.user_id=u.id  WHERE o.id= $id");
    ...
    { //in admin order
        $stm = $this->db->prepare("SELECT u.username, u.full_name FROM user u INNER JOIN ordered o ON o.user_id=u.id WHERE o.id = $id");
        $stm->setFetchMode(PDO::FETCH_ASSOC);
        $stm->execute();
        return $stm->fetch();
    }
